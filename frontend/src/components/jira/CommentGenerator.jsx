...
const PLACEHOLDER_FORMAT = `Validation Status: [Pass / Fail / Partial Pass]
Tested By: [Your Name]
...
export default function CommentGenerator({ config, template, onSaveTemplate, getHeaders }) {
  ...
  const handleSaveTemplate = () => {
    ...
    onSaveTemplate("commentFormat", draftTemplate.trim());
  ...
  return (
    <div className="flex flex-col h-full gap-4">
      ...
      <div className="bg-[#1a1f2e] border border-gray-700 rounded-xl overflow-hidden">
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-700">
          <div className="flex items-center gap-2">
            <span className="text-sm font-semibold text-white">🏢 Comment Format</span>
        ...
        {editingTemplate ? (
          <div className="p-4">
            <p className="text-xs text-gray-400 mb-2">Paste your company's JIRA comment format. AI will always use it.</p>
            <textarea value={draftTemplate} onChange={(e) => setDraftTemplate(e.target.value)} rows={8}
            ...
            <button onClick={handleSaveTemplate} disabled={!draftTemplate.trim()}
              className="mt-2 px-4 py-2 bg-violet-600 hover:bg-violet-500 disabled:bg-gray-700 text-white text-sm rounded-lg">Save Format</button>
          </div>
        ) : template ? (
          <pre className="px-4 py-3 text-xs text-gray-400 font-mono whitespace-pre-wrap max-h-24 overflow-y-auto">{template}</pre>
        ) : (
          <div className="px-4 py-3 text-sm text-amber-400">⚠ No format saved. Click "Edit" first.</div>
        )}
      </div>
      ...
      <div className="bg-[#1a1f2e] border border-gray-700 rounded-xl p-4">
        ...
        <textarea value={validationNotes} onChange={(e) => setValidationNotes(e.target.value)} rows={4}
        ...
        <div className="flex items-center gap-3 mt-3">
          <button onClick={handleGenerate} disabled={isLoading || !validationNotes.trim() || !template}
            ...
            {isLoading ? <><svg className="animate-spin w-4 h-4" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" className="opacity-25"/><path fill="currentColor" className="opacity-75" d="M4 12a8 8 0 018-8v8H4z"/></svg>Generating...</> : "✨ Generate Comment"}
          </button>
            ...
            <div className="px-4 pb-4 border-t border-gray-700 pt-3">
              <div className="flex items-center gap-3">
                <button onClick={handlePost} disabled={posting}
                  ...
                  {posting ? <><svg className="animate-spin w-4 h-4" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" className="opacity-25"/><path fill="currentColor" className="opacity-75" d="M4 12a8 8 0 018-8v8H4z"/></svg>Posting...</> : `🔵 Post to ${ticketId}`}
                </button>
